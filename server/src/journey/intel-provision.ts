...
/**
 * One-time setup for the CubeSmart real-time Conversation Intelligence layer.
 *
 * This is the in-server equivalent of the old one-off `provision-cintel.cjs`
 * script. It has to live here rather than as a standalone script because it
 * needs the real account credentials that only exist on the running server —
 * everything is idempotent by display name, so calling it again is always
 * safe and just confirms what already exists.
 *
 * Creates:
 *   1. A Knowledge base with the store's retention playbook as raw text
 *   2. Three custom Language Operators — call reason, retention risk, next best action
 *   3. An intelligence configuration with two rules posting results to /journey/cintel
 *   4. Attaches that configuration to the live Conversation Orchestrator config
 */
...
/* ------------------------------------------------------------------ *
 * 1. Knowledge — the store's own retention playbook
 * ------------------------------------------------------------------ */
...
async function ensureKnowledge(): Promise<{ baseId: string; sourceId: string }> {
  const bases = await call("GET", `${KNOW}/ControlPlane/KnowledgeBases`);
  let base = (bases.knowledgeBases || []).find((b: any) => b.displayName === KB_NAME);
  ...
  if (!base) {
    ...
    if (!base?.id) {
      ...
      base = (again.knowledgeBases || []).find((b: any) => b.displayName === KB_NAME);
    }
  }
  if (!base?.id) throw new Error("Knowledge base was not created");

  const sources = await call("GET", `${KNOW}/KnowledgeBases/${base.id}/Knowledge`);
  const existingSources: any[] = sources.knowledge || sources.items || [];
  ...
  if (!source) {
    const attempts = [{ type: "Text", content: PLAYBOOK }, { content: PLAYBOOK }];
    ...
    for (const src of attempts) {
      try {
        ...
        if (!source?.id) {
          ...
          source = (again.knowledge || again.items || []).find((s: any) => s.name === SOURCE_NAME);
        }
        if (source?.id) break;
      } catch (err) {
        lastError = err;
...
const OPERATORS = [
  {
    ...
    outputSchema: {
      ...
      properties: {
        ...
        confidence: { type: "number", description: "How certain you are, from 0 to 1." },
        evidence: {
          type: "string",
...
- Keep evidence to a short quote of the tenant's words. Leave it empty when the reason is "Not clear yet".`,
  },
  {
    ...
    outputSchema: {
      type: "object",
      properties: {
        score: { type: "integer", description: "Retention risk from 0 (no risk) to 100 (about to leave)." },
        ...
        quote: { type: "string", description: "The single most telling thing the tenant said. Empty if nothing yet." },
...
Start low and only move as evidence appears. Set trend by comparing with how the conversation opened. Keep each driver under eight words, phrased as a fact about the tenant. Return an empty drivers list and an empty quote ONLY while the score is still in the low band — from 25 upwards you must always name at least one driver and quote the tenant's own words that moved the score.`,
  },
  {
    ...
    outputSchema: {
      type: "object",
      properties: {
        recommend: { type: "boolean", description: "True only when there is a genuine reason to make an offer right now." },
        headline: { type: "string", description: 'Six words or fewer naming the save, e.g. "20% discount for 3 months".' },
        offer: { type: "string", description: "Exactly what to offer, in words the agent could say out loud." },
        rationale: { type: "string", description: "One sentence on why this tenant, right now." },
        policy_source: { type: "string", description: "The part of the store playbook this comes from." },
...
async function ensureOperators(): Promise<Record<string, string>> {
  ...
  const byName = new Map((existing.items || []).map((o: any) => [o.displayName, o]));
  const ids: Record<string, string> = {};

  for (const spec of OPERATORS) {
    const found: any = byName.get(spec.displayName);
    if (found) {
...
function buildRules(operatorIds: Record<string, string>, knowledge: { baseId: string }, webhookUrl: string) {
  const action = { type: "WEBHOOK", method: "POST", url: webhookUrl };
  const memoryOnly = { memory: { enabled: true } };
  const withKnowledge = { memory: { enabled: true }, knowledge: { bases: [knowledge.baseId] } };

  return [
    {
      operators: [{ id: operatorIds["CubeSmart Call Reason"] }, { id: TWILIO_SENTIMENT }],
...
async function ensureConfiguration(
  ...
  knowledge: { baseId: string },
  webhookUrl: string
): Promise<string> {
  const list = await call("GET", `${INTEL}/Configurations`);
  const found = (list.items || []).find((c: any) => c.displayName === CONFIG_NAME);
  const payload = {
    displayName: CONFIG_NAME,
    description:
      "Live call reason, sentiment, retention risk and next best action for the CubeSmart guided move-in journey.",
    rules: buildRules(operatorIds, knowledge, webhookUrl),
  };
...
async function attachToConversationConfiguration(configId: string): Promise<boolean> {
  const convConfigId = process.env.TWILIO_CONVERSATION_CONFIGURATION_ID;
  if (!convConfigId) return false;

  const list = await call("GET", `${CONV}/Configurations`);
  const current = (list.configurations || []).find((c: any) => c.id === convConfigId);
  if (!current) return false;

  const before: string[] = current.intelligenceConfigurationIds || [];
  if (before.includes(configId)) return true;
  ...
  await call("PUT", `${CONV}/Configurations/${convConfigId}`, body);
  return true;
}
...
export interface CintelProvisionResult {
  knowledgeBaseId: string;
  operatorIds: Record<string, string>;
  sentimentOperatorId: string;
  configId: string;
  attached: boolean;
  webhookUrl: string;
}

export async function provisionCintel(): Promise<CintelProvisionResult> {
  const domain = process.env.TWILIO_VOICE_PUBLIC_DOMAIN;
  if (!domain) throw new Error("TWILIO_VOICE_PUBLIC_DOMAIN is not set.");
  const webhookUrl = `https://${domain}/journey/cintel`;

  const knowledge = await ensureKnowledge();
  const operatorIds = await ensureOperators();
  const configId = await ensureConfiguration(operatorIds, knowledge, webhookUrl);
  const attached = await attachToConversationConfiguration(configId);

  return {
    knowledgeBaseId: knowledge.baseId,
    operatorIds,
    sentimentOperatorId: TWILIO_SENTIMENT,
    configId,
    attached,
    webhookUrl,
  };
